...
const tokenCooldownUntil = new Map<string, number>()   // token → ts until which it's considered throttled
...
const showboxCache = new Map<string, { sources: Source[]; ts: number }>()
...
async function fbQualityListPooled(fid: string, shareKey: string, fileName: string): Promise<string> {
  const hdExpected = /\b(1080|2160|4k|720|blu-?ray|web-?dl|remux|hdr)\b/i.test(fileName)
  let bestHtml = ''
  for (const tok of liveTokens()) {
    const qData: any = await fbFetchJson(`${FEBBOX}/console/video_quality_list?fid=${fid}&share_key=${shareKey}`, fbHeaders(`ui=${tok}`))
    ...
    bestHtml = html
    if (hasHd || !hdExpected) break                              // got HD, or genuinely low-res → done
...
async function getShowboxSources(tmdb: string, type: string, season: string, episode: string): Promise<Source[]> {
  ...
  if (cached && Date.now() - cached.ts < SHOWBOX_CACHE_TTL) return cached.sources
  ...
  try {
    ...
    if (sources.some(s => /1080|2160|4K|720|ORG/i.test(s.lang))) showboxCache.set(cacheKey, { sources, ts: Date.now() })
    return sources
  } catch {
    return []
  }
