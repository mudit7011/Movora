...
function seal(u: string, r: string, seg = false): string {
...
function unseal(token: string): { u: string; r: string; s: number } | null {
  try {
...
const playUrl = (u: string, r: string, seg = false) => `/api/stream/hls?d=${seal(u, r, seg)}`
...
async function fbFetchJson(url: string, headers: any, tries = 3, viaProxy = false): Promise<any | null> {
  ...
  for (let i = 0; i < tries; i++) {
    try { const j = await attempt(useProxy); if (j) return j } catch { /* retry */ }
  ...
  if (useProxy) { try { const j = await attempt(false); if (j) return j } catch { /* */ } }
  return null
}
  ...
  try {
    ...
    const perFile = await Promise.all(files.map(async (f: any): Promise<Source[]> => {
      try {
        ...
        const qData: any = await fbFetchJson(`${FEBBOX}/console/video_quality_list?fid=${f.fid}&share_key=${shareKey}`, fbHeaders(cookie), 3, true)
...
streamRouter.get('/fbdebug', async (_req, res) => {
  const out: any = {
  ...
  if (out.egressIp && !String(out.egressIp).startsWith('err')) {
    try { const g: any = await (await fetch(`http://ip-api.com/json/${out.egressIp}?fields=country,city`, { signal: AbortSignal.timeout(6000) })).json(); out.egressGeo = `${g.city}, ${g.country}` } catch {}
  }
