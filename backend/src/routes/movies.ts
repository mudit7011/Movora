...
router.get('/search', async (req, res) => {
  try {
    ...
    if (deduped.length < 4) {
      try {
        const td: any = await tmdbFetch(`/search/movie?query=${encodeURIComponent(raw)}&language=en-US&page=1`)
        const found: any[] = (td?.results || []).filter((r: any) => r.poster_path).slice(0, 8)
        const ids = found.map(r => String(r.id))
        const have = new Set((await Movie.find({ tmdbId: { $in: ids } }).select('tmdbId').lean()).map((m: any) => String(m.tmdbId).replace(/^movie_/, '')))
        const missing = found.filter(r => !have.has(String(r.id)))
        for (let i = 0; i < missing.length; i += 5) {
          await Promise.allSettled(missing.slice(i, i + 5).map(r =>
            importMovie(r.id, { bypassGate: r.genre_ids?.includes(16) })))   // anime → skip gate
        }
        const docs = await Movie.find({ tmdbId: { $in: ids }, type: 'movie' }).select('-sources').lean()
        const byId = new Map(docs.map((d: any) => [String(d.tmdbId).replace(/^movie_/, ''), d]))
