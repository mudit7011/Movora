...
router.get('/actor', async (req, res) => {
  try {
    ...
    if (q.length < 2) { res.json({ person: null, results: [] }); return }
    ...
    const search = await tmdbFetch(`/search/person?query=${encodeURIComponent(q)}&language=en-US&page=1`)
    ...
    const docs = await Movie.find({ tmdbId: { $in: ids }, posterUrl: { $ne: '' } }).select('-sources').lean()
    ...
    cacheSet(cache, q.toLowerCase(), { data, ts: Date.now() }, CACHE_MAX)
    res.json(data)
  } catch {
