...
const EMBED_SERVERS = [
  (id: string) => ({ name: 'Server 1', url: `https://player.videasy.net/tv/${id}`,        type: 'iframe', quality: 'HD' }),
  (id: string) => ({ name: 'Server 2', url: `https://www.2embed.cc/embedtv/${id}`,        type: 'iframe', quality: 'HD' }),
  (id: string) => ({ name: 'Server 3', url: `https://vidsrc.icu/embed/tv/${id}`,          type: 'iframe', quality: 'HD' }),
  (id: string) => ({ name: 'Server 4', url: `https://embed.su/embed/tv/${id}`,            type: 'iframe', quality: 'HD' }),
  (id: string) => ({ name: 'Server 5', url: `https://vidsrc.cc/v2/embed/tv/${id}`,        type: 'iframe', quality: 'HD' }),
]
...
async function collectCatalog(maxPages = 0): Promise<CatalogItem[]> {
  ...
  const all: CatalogItem[] = [...first.items]
...
async function enrichFromTmdb(tmdbId: string) {
  ...
  const title: string = detail.name || detail.original_name || ''
  ...
  const cast = ((credits.cast as any[]) ?? []).slice(0, 15).map((c: any) => ({
    name: c.name,
  ...
  const runtime = (detail.episode_run_time as number[] ?? [])[0] ?? 45

  const seasonData = ((detail.seasons as any[]) ?? [])
    .filter((s: any) => s.season_number > 0)
    .map((s: any) => ({ seasonNumber: s.season_number, episodeCount: s.episode_count, name: s.name }))
...
async function main() {
  if (!TMDB_BEARER) { console.error('TMDB_BEARER not set'); process.exit(1) }
