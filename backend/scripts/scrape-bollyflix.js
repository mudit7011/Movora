...
async function tmdbSearch(title, year) {
    try {
        ...
        const res = await fetch(`${TMDB_BASE}/search/movie?query=${q}&year=${year}&language=en-US&include_adult=false`, { headers: { Authorization: `Bearer ${BEARER}`, Accept: 'application/json' } });
        if (!res.ok)
            return null;
        const data = await res.json();
        if (!data.results?.length) {
            // retry without year constraint
            const res2 = await fetch(`${TMDB_BASE}/search/movie?query=${q}&language=en-US&include_adult=false`, { headers: { Authorization: `Bearer ${BEARER}`, Accept: 'application/json' } });
...
async function tmdbDetail(id) {
    try {
        const res = await fetch(`${TMDB_BASE}/movie/${id}?language=en-US&append_to_response=credits,videos`, { headers: { Authorization: `Bearer ${BEARER}`, Accept: 'application/json' } });
        if (!res.ok)
            return null;
        return res.json();
    }
    ...
    for (const [category, langs] of CATEGORIES) {
        ...
        for (let page = 1; page <= MAX_PAGES; page++) {
            ...
            for (const raw of rawTitles) {
                ...
                if (!doc.posterUrl || !doc.backdropUrl)
                    continue; // skip no-image movies
                await col.updateOne({ slug }, { $setOnInsert: doc }, { upsert: true });
                totalAdded++;
