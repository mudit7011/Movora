...
async function tmdbGet(endpoint: string): Promise<any> {
...
async function omdbGet(imdbId: string): Promise<number | null> {
...
async function main() {
  ...
  if (OMDB_KEY) console.log('OMDB key found — will use real IMDb ratings\n')
  else          console.log('No OMDB key — using TMDB vote_average (add OMDB_API_KEY to .env for real IMDb ratings)\n')
  ...
  for (let i = 0; i < movies.length; i++) {
    ...
    try {
      ...
      const update: Record<string, any> = { rating, runtime, updatedAt: new Date() }
      if (releaseYear) update.releaseYear = releaseYear
      ...
      process.stdout.write(
        `\r[${i + 1}/${movies.length}] ${updated} updated, ${failed} failed — last: ${movie.title} → ${rating} (${source})   `
      )
