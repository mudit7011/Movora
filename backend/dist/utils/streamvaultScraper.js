...
exports.scrapeStreamVault = scrapeStreamVault;
...
function clearStreamCache() { cache.clear(); }
function cacheKey(tmdbId, type, season, episode) {
    return type === 'movie' ? `movie_${tmdbId}` : `tv_${tmdbId}_s${season}_e${episode}`;
}
async function scrapeStreamVault(tmdbId, type, season, episode, forceRefresh = false) {
    ...
    try {
        ...
        let streamUrl = null; // master.m3u8 — top priority
        let url1080 = null;
        let url720 = null;
        let url680 = null;
        page.on('request', req => {
            const url = req.url();
            if (!url.includes('streamvaultsrc.click/stream-proxy/pl') || !url.includes('.m3u8'))
                return;
        ...
        const subtitles = [{ label: 'EN', language: 'en', url: subUrl, default: true }];
        const stream = { url: finalUrl, subtitles };
