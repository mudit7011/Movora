...
exports.tmdbToAnilist = tmdbToAnilist;
...
async function loadFribb() {
    if (fribbCache && Date.now() - fribbCache.ts < FRIBB_TTL)
        return fribbCache.byTmdb;
    try {
        const r = await fetch(FRIBB_URL, { signal: AbortSignal.timeout(15000) });
        ...
        if (fribbCache)
            return fribbCache.byTmdb; // keep stale on fetch failure
        return new Map();
    }
}
...
async function tmdbToAnilist(tmdb, type, season = 1) {
...
function normalize(provider, raw) {
    ...
    const proxy = (u) => `/api/subtitles/anime?u=${Buffer.from(u, 'utf8').toString('base64url')}`;
    const subsOf = (arr) => (arr || []).filter(s => s?.url).map(s => ({ url: proxy(s.url), label: s.label || s.lang || 'Subs', lang: s.srclang || s.lang || 'en' }));
    ...
    if (!s?.url || !/\.(m3u8|mp4)/i.test(s.url))
        return null;
...
async function getAnimeSources(type, season, episode, tmdb) {
    if (!ANIME_API_URL)
        return [];
    try {
        const anilist = await tmdbToAnilist(tmdb, type, Number(season) || 1);
        if (!anilist)
            return []; // not a known anime → skip
        ...
        const results = await Promise.all(PROVIDER_PRIORITY.map(async (p) => {
            try {
                const er = await fetch(`${ANIME_API_URL}/episodes/${p}/${anilist}`, { signal: AbortSignal.timeout(20000) });
                if (!er.ok)
                    return null;
                const eps = await er.json().catch(() => null);
                const arr = eps?.[p]?.episodes?.sub || eps?.[p]?.episodes?.dub || [];
                const epObj = arr.find(e => e.number === ep) || (ep === 1 ? arr[0] : null);
                if (!epObj?.id)
                    return null;
                const wr = await fetch(`${ANIME_API_URL}/${epObj.id}`, { signal: AbortSignal.timeout(20000) });
                if (!wr.ok)
                    return null;
        ...
        for (const src of results)
            if (src && !out.some(o => o.url === src.url)) {
                out.push(src);
                if (out.length >= 3)
                    break;
            }
