...
router.get('/search', async (req, res) => {
    ...
    if (!mapped) {
        ...
        if (r && r.ok) {
            ...
            let first = ((d?.data) || []).map(mapItem).filter((s) => s.id);
            ...
            if (first.length === 0 && type === 'tv' && season && episode && await (0, anime_1.tmdbToAnilist)(tmdb, 'tv', Number(season)).catch(() => null)) {
                ...
                if (r2 && r2.ok) {
                    ...
                    first = ((d2?.data) || []).map(mapItem).filter((s) => s.id);
                }
            }
            mapped = first;
...
router.get('/anime', async (req, res) => {
    ...
    if (hit && Date.now() - hit.ts < VTT_TTL) {
        serve(hit.vtt);
        return;
    }
    try {
        ...
        if (animeSubCache.size > 800) {
            const o = [...animeSubCache.entries()].sort((a, b) => a[1].ts - b[1].ts)[0];
            animeSubCache.delete(o[0]);
        }
        serve(vtt);
    }
    catch {
