...
router.get('/search', async (req, res) => {
    ...
    if (!API_KEY)
        return res.status(503).json({ error: 'Subtitles service not configured' });
    ...
    if (!r?.ok)
        return res.status(502).json({ error: 'OpenSubtitles search failed' });
    const data = await r.json();
    const results = (data.data ?? []).slice(0, 20).map((item) => {
        const a = item.attributes;
        const files = a.files?.[0];
        return {
...
router.post('/download', async (req, res) => {
    ...
    if (!r?.ok)
        return res.status(502).json({ error: 'Download link request failed' });
    const data = await r.json();
    if (!data.link)
        return res.status(502).json({ error: 'No download link returned' });
    // Proxy the actual subtitle file so the browser doesn't hit CORS
    const sub = await fetch(data.link).catch(() => null);
    if (!sub?.ok)
        return res.status(502).json({ error: 'Subtitle file fetch failed' });
