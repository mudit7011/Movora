...
async function freshSeasons(rawId) {
    const hit = seasonCache.get(rawId);
    if (hit && Date.now() - hit.ts < SEASON_TTL)
        return hit.data;
    try {
        const d = await (0, tmdb_1.tmdbFetch)(`/tv/${rawId}?language=en-US`);
        if (!Array.isArray(d?.seasons))
            return null;
        const seasonData = d.seasons
            .filter((s) => s.season_number > 0 && s.episode_count > 0) // skip specials + unaired-empty seasons
            .map((s) => ({ seasonNumber: s.season_number, episodeCount: s.episode_count, name: s.name || `Season ${s.season_number}` }));
        if (!seasonData.length)
            return null;
        const data = { seasonData, seasons: seasonData.length };
        seasonCache.set(rawId, { data, ts: Date.now() });
        if (seasonCache.size > 500) {
            const o = [...seasonCache.entries()].sort((a, b) => a[1].ts - b[1].ts)[0];
            seasonCache.delete(o[0]);
        }
        return data;
    }
    catch {
...
router.get('/related/:slug', async (req, res) => {
    try {
        ...
        const show = await Movie_1.Movie.findOne({ slug: req.params.slug, type: 'tvshow' }).select('_id tmdbId genres language rating');
        if (!show)
        ...
        async function dbMatches(tmdbItems) {
            ...
            const existing = await Movie_1.Movie.find({ tmdbId: { $in: ids }, type: 'tvshow' }).select('-sources').lean();
            const byId = new Map(existing.map((m) => [String(m.tmdbId).replace(/^tv_/, ''), m]));
            return tmdbItems
                .map(r => byId.get(String(r.id)))
                .filter((d) => !!d && !!d.posterUrl && String(d._id) !== showId);
        }
        let recMatches = [];
        let simMatches = [];
        if (rawId) {
            const [recData, simData] = await Promise.allSettled([
                (0, tmdb_1.tmdbFetch)(`/tv/${rawId}/recommendations?language=en-US&page=1`),
                (0, tmdb_1.tmdbFetch)(`/tv/${rawId}/similar?language=en-US&page=1`),
            ]);
        ...
        const [genrePool, broadPool] = await Promise.all([
            Movie_1.Movie.find({ _id: { $ne: show._id }, type: 'tvshow', genres: { $in: topGenres }, language: { $in: show.language }, streamVerified: { $ne: false }, rating: { $gte: 5 }, posterUrl: { $ne: '' } }).sort({ rating: -1, releaseYear: -1 }).limit(80).select('-sources').lean(),
            Movie_1.Movie.find({ _id: { $ne: show._id }, type: 'tvshow', genres: { $nin: topGenres }, language: { $in: show.language }, streamVerified: { $ne: false }, rating: { $gte: 6.5 }, posterUrl: { $ne: '' } }).sort({ rating: -1, releaseYear: -1 }).limit(80).select('-sources').lean(),
        ]);
        const seen = new Set([rawId]); // never include the show itself
        const result = {
...
router.get('/reviews/:slug', async (req, res) => {
    try {
        ...
        if (cached && Date.now() - cached.ts < REVIEWS_TTL)
            return res.json(cached.data);
        const show = await Movie_1.Movie.findOne({ slug: req.params.slug, type: 'tvshow' }).select('tmdbId');
        if (!show)
        ...
        if (!rawId)
            return res.json({ reviews: [] });
        const data = await (0, tmdb_1.tmdbFetch)(`/tv/${rawId}/reviews?language=en-US&page=1`).catch(() => null);
        const reviews = (data?.results || []).slice(0, 8).map((r) => ({
            id: String(r.id),
        ...
        (0, boundedCache_1.cacheSet)(reviewsCache, req.params.slug, { data: result, ts: Date.now() }, RELATED_MAX);
        res.json(result);
    }
    catch {
        res.json({ reviews: [] });
...
router.get('/:slug', async (req, res) => {
    try {
        ...
        if (fresh) {
            obj.seasonData = fresh.seasonData;
            obj.seasons = fresh.seasons;
        }
        res.json(obj);
    }
    catch {
