...
exports.getMovieHash = getMovieHash;
...
async function tmdbTitle(tmdb, type) {
    try {
        const d = await (0, tmdb_1.tmdbFetch)(`/${type === 'tv' ? 'tv' : 'movie'}/${tmdb}?language=en-US`);
...
function titleMatches(expected, filename) {
    const norm = (s) => (s || '').toLowerCase().replace(/[^a-z0-9]+/g, ' ');
    const words = norm(expected).split(' ').filter(w => w.length >= 4 && !TITLE_STOP.has(w));
    if (words.length === 0)
        return true; // nothing distinctive → can't verify, allow
...
function unseal(token) {
    try {
        ...
        if (raw.length < 29)
            return null;
        const d = crypto_1.default.createDecipheriv('aes-256-gcm', STREAM_SECRET, raw.subarray(0, 12));
...
async function getApiKey() {
    if (apiKeyCache && Date.now() - apiKeyCache.ts < KEY_TTL)
        return apiKeyCache.key;
    try {
        const r = await fetch(KEY_URL, { headers: H, signal: AbortSignal.timeout(10000) });
        if (!r.ok)
            return null;
        ...
        const keyHash = crypto_1.default.createHash('sha256').update(ENCRYPTION_KEY, 'utf8').digest();
        const decipher = crypto_1.default.createDecipheriv('aes-256-gcm', keyHash, iv);
...
function decryptLink(link, apiKey) {
    try {
...
async function fetchServer(sr, params, apiKey) {
    try {
        const r = await fetch(`${SERVER_BASE}?${params}&sr=${sr}`, { headers: H, signal: AbortSignal.timeout(5000) });
        if (!r.ok)
            return [];
...
async function getShowboxSources(tmdb, type, season, episode) {
    ...
    try {
        ...
        if (type !== 'tv' && expectedTitle && files.length > 0 && !files.some((f) => titleMatches(expectedTitle, f.file_name))) {
            console.warn(`[ShowBox] title mismatch — expected "${expectedTitle}", got "${files[0]?.file_name}". Rejecting wrong id ${showboxId}.`);
            return [];
        }
        if (type === 'tv') {
            const folder = files.find((f) => f.file_name?.toLowerCase() === `season ${season}`);
            if (!folder)
                return [];
            const eps = await listFiles(folder.fid);
            ...
            if (expectedTitle && files.length > 0 && !files.some((f) => titleMatches(expectedTitle, f.file_name))) {
                console.warn(`[ShowBox] TV title mismatch — expected "${expectedTitle}", got "${files[0]?.file_name}". Rejecting.`);
                return [];
            }
        }
        files = files.filter((f) => f.fid && !f.is_dir && /\.(mp4|mkv|avi|m3u8)$/i.test(f.file_name || '')).slice(0, 3);
...
function osdbHash(head, tail, size) {
    const MASK = (1n << 64n) - 1n;
    let h = BigInt(size) & MASK;
    for (let i = 0; i + 8 <= head.length; i += 8)
        h = (h + head.readBigUInt64LE(i)) & MASK;
    for (let i = 0; i + 8 <= tail.length; i += 8)
        h = (h + tail.readBigUInt64LE(i)) & MASK;
    return h.toString(16).padStart(16, '0');
}
async function osdbHashFromUrl(url, cookie) {
    try {
        const hdr = { 'User-Agent': UA, Referer: FEBBOX_REFERER, Cookie: cookie };
        const headR = await fetch(url, { headers: { ...hdr, Range: 'bytes=0-65535' }, redirect: 'follow', signal: AbortSignal.timeout(6000) });
        if (!headR.ok && headR.status !== 206)
            return null;
        ...
        if (!size || size < 131072)
            return null; // too small / unknown size
        const head = Buffer.from(await headR.arrayBuffer());
        const tailR = await fetch(url, { headers: { ...hdr, Range: `bytes=${size - 65536}-${size - 1}` }, redirect: 'follow', signal: AbortSignal.timeout(6000) });
        if (!tailR.ok && tailR.status !== 206)
            return null;
        const tail = Buffer.from(await tailR.arrayBuffer());
        if (head.length < 65536 || tail.length < 65536)
            return null;
...
async function getMovieHash(tmdb, type, season, episode) {
    if (!FEBBOX_UI)
        return null;
    const key = `${type}:${tmdb}:${season}:${episode}`;
    const hit = hashCache.get(key);
    if (hit && Date.now() - hit.ts < HASH_TTL)
        return hit.hash;
    const put = (h) => { hashCache.set(key, { hash: h, ts: Date.now() }); return h; };
    const cookie = FEBBOX_UI.startsWith('ui=') ? FEBBOX_UI : `ui=${FEBBOX_UI}`;
    const T = 8000;
    try {
        const idUrl = type === 'tv'
        ...
        const file = files.find((f) => f.fid && !f.is_dir && /\.(mp4|mkv|avi)$/i.test(f.file_name || ''));
        if (!file)
            return put(null);
        // ORG = the untouched release file that community subs were synced against.
        const qr = await fetch(`${FEBBOX}/console/video_quality_list?fid=${file.fid}&share_key=${shareKey}`, { headers: fbHeaders(cookie), signal: AbortSignal.timeout(8000) });
        ...
        for (const block of html.split('file_quality').slice(1)) {
            ...
            if (u && /^https?:\/\//i.test(u) && qual === 'ORG' && /\.(mp4|mkv|avi)(\?|$)/i.test(u)) {
                orgUrl = u;
                break;
            }
        }
...
exports.streamRouter.get('/', async (req, res) => {
    ...
    if (!tmdb) {
        res.status(400).json({ error: 'tmdb id required' });
        return;
    }
    const cacheKey = `${type}:${tmdb}:${season}:${episode}`;
    const hit = srcCache.get(cacheKey);
    if (hit && Date.now() - hit.ts < SRC_TTL) {
        res.json({ sources: sealAll(hit.data) });
        return;
    }
    let params = `id=${encodeURIComponent(tmdb)}`;
    if (type === 'tv' && season && episode)
        params += `&ss=${encodeURIComponent(season)}&ep=${encodeURIComponent(episode)}`;
    ...
    if (sources.length === 0) {
        res.status(502).json({ error: 'no sources', sources: [] });
        return;
    }
...
exports.streamRouter.get('/hls', async (req, res) => {
