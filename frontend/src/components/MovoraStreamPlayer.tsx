...
export default function MovoraStreamPlayer({ tmdb, type, season, episode, title, poster, year, runtime, rating, synopsis, sourceIdx, onSourceIdxChange, onSourcesList, onFallback, startAt, onProgress }: Props) {
  ...
  const [episodeInfo, setEpisodeInfo] = useState<{ title: string; overview: string } | null>(null)

  // For TV, grab the current episode's title + overview (Netflix-style info overlay).
  useEffect(() => {
    if (type !== 'tv' || !season || !episode) { setEpisodeInfo(null); return }
    let cancelled = false
    fetch(`/api/episodes?tmdbId=${encodeURIComponent(tmdb)}&season=${season}`)
      .then(r => r.ok ? r.json() : [])
      .then((eps: { episodeNumber: number; name: string; overview: string }[]) => {
        if (cancelled) return
        const ep = Array.isArray(eps) ? eps.find(e => e.episodeNumber === episode) : null
        setEpisodeInfo(ep ? { title: ep.name, overview: ep.overview } : null)
      })
      .catch(() => {})
  ...
  useEffect(() => {
    let cancelled = false
    setSources(null); setDead(false); setSubs([])
