  ...
  return (
    <section className="py-8 relative">
      ...
      <div className={`flex items-center justify-between mb-6 px-4 sm:px-6 ${isTV ? 'lg:px-16' : 'lg:pl-24 lg:pr-8'}`}>
        <div>
          <h2 className="text-xl sm:text-2xl font-semibold text-foreground">Continue Watching</h2>
        ...
        <div className="hidden sm:flex items-center gap-2">
          <button
            ...
            className={`p-2 rounded-full bg-white/5 border border-white/10 text-foreground transition-all duration-300 ${showLeftArrow ? 'hover:bg-white/10 hover:border-white/20' : 'opacity-30 cursor-not-allowed'}`}
          ...
          <button
            ...
            className={`p-2 rounded-full bg-white/5 border border-white/10 text-foreground transition-all duration-300 ${showRightArrow ? 'hover:bg-white/10 hover:border-white/20' : 'opacity-30 cursor-not-allowed'}`}
      ...
      {/* Carousel — native scroll for smooth mobile touch */}
      <div className="relative">
        <div className={`absolute left-0 top-0 bottom-0 w-16 bg-gradient-to-r from-background to-transparent z-10 pointer-events-none transition-opacity duration-300 ${showLeftArrow ? 'opacity-100' : 'opacity-0'}`} />
        <div className={`absolute right-0 top-0 bottom-0 w-16 bg-gradient-to-l from-background to-transparent z-10 pointer-events-none transition-opacity duration-300 ${showRightArrow ? 'opacity-100' : 'opacity-0'}`} />

        <div
          ref={scrollRef}
          className={`flex gap-4 pb-4 overflow-x-auto overscroll-x-contain scroll-smooth
